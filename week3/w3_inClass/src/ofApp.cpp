...
void ofApp::draw(){
  ...
  vector<Real> spectrum = mltk.getData("Spectrum");
    ...
    hpcpSmooth.assign(hpcpUnsmooth.size(), 0);
    for(int i=0; i<hpcpUnsmooth.size(); i++){
        hpcpSmooth[i] = ofInterpolateCosine(hpcpUnsmooth[i], hpcpSmooth[i], 0.5)
    }
  ...
  float hpcpWidth = (ofGetWidth() / float(hpcp.size()));
    ...
    if(showBands){
        for(int i = 0; i < hpcpSmooth.size(); i++){
            ofDrawRectangle(i * hpcpWidth,
                            ofGetHeight(),
                            hpcpWidth,
                            -ofMap(hpcp[i], 0, 1.0, 0, ofGetHeight(), true)*bandsNoveltyFactor.get());
        }
    }
